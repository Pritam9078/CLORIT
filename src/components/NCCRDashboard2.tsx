...
import { nccrApprove } from '../services/blockchain';
...
const NCCRDashboard: React.FC<NCCRDashboardProps> = ({
  ...
  const handleProjectApproval = async (projectId: string, action: 'approve' | 'reject') => {
    ...
    try {
      const project = nCCRProjects.find(p => p.id === projectId);
      ...
      await nccrApprove(projectId, dummyDataHash, credits, metadataURI, approve);
      
      alert(`Project ${projectId} has been ${approve ? 'finally approved and credits minted' : 'rejected'} on-chain!`);
    } catch (error: any) {
      console.error(error);
      alert(`NCCR Action failed: ${error.message}`);
    } finally {
                ...
                <div style={styles.actionButtons}>
                  <button
                    ...
                    style={{ 
                      ...
                      opacity: isProcessing === project.id ? 0.7 : 1
                    }}
                    onClick={() => handleProjectApproval(project.id, 'approve')}
                  >
                    {isProcessing === project.id ? <Loader2 className="w-4 h-4 animate-spin" /> : '✅ Approve & Mint'}
                  </button>
                  <button
                    disabled={isProcessing === project.id}
                    style={{ 
