...
import { panchayatApprove } from '../services/blockchain';
...
const PanchayatDashboard = () => {
  ...
  const handleApprove = async (projectId: string) => {
    ...
    try {
      const dummyDataHash = "0x" + "2".repeat(64); 
      await panchayatApprove(projectId, dummyDataHash, true);
      alert(`Project ${projectId} approved by Panchayat on-chain!`);
    } catch (error: any) {
      console.error(error);
  ...
  const handleReject = async (projectId: string) => {
    ...
    try {
      const dummyDataHash = "0x" + "2".repeat(64); 
      await panchayatApprove(projectId, dummyDataHash, false);
      alert(`Project ${projectId} rejected on-chain. Reason: ${reason}`);
    } catch (error: any) {
      console.error(error);
                        ...
                        <td style={styles.tableCell}>
                          ...
                          <button
                            ...
                            style={{ 
                              ...
                              opacity: isProcessing === project.id ? 0.5 : 1
                            }}
                            onClick={() => handleApprove(project.id)}
                            onMouseEnter={(e) => (e.currentTarget.style.background = '#059669')}
                            onMouseLeave={(e) => (e.currentTarget.style.background = '#10b981')}
                          >
                            {isProcessing === project.id ? '...' : 'Approve'}
                          </button>
                          <button
                            disabled={isProcessing === project.id}
                            style={{ 
