...
const PanchayatNav: React.FC = () => {
    ...
    return (
        <header className="sticky top-0 z-[100] w-full bg-white/90 backdrop-blur-xl border-b border-purple-100 shadow-sm">
            ...
            {/* Mobile Menu Content */}
            {isMobileMenuOpen && (
                <div className="xl:hidden bg-white border-t border-purple-50 px-4 py-6 space-y-4 animate-in slide-in-from-top-4 duration-300 shadow-xl">
                        ...
                        {walletState?.isConnected ? (
                            <div className="flex items-center justify-between px-4 py-3 bg-purple-50 border border-purple-100 rounded-xl">
                                <div className="flex items-center gap-2">
                                    <div className="w-2 h-2 bg-purple-500 rounded-full animate-pulse" />
                                    <div>
                                        <p className="text-xs font-bold text-purple-700">{WalletUtils.formatAddress(walletState.address!)}</p>
                                        <p className="text-xs text-purple-600">{parseFloat(walletState.balance).toFixed(3)} ETH</p>
                                    </div>
                                </div>
                                <button
                                    onClick={handleWalletDisconnect}
                    ...
                    <div className="pt-4 border-t border-slate-100 space-y-2">
                        <div className="px-4 py-2">
                            ...
                            <p className="text-sm font-bold text-slate-800 mt-1">{currentUser?.panchayatName || 'Panchayat Admin'}</p>
                        </div>
                        <button
                            onClick={() => {
                                navigate('/panchayat-profile');
