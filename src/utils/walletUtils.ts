...
export const WalletUtils = {
  ...
  disconnectWallet: (): void => {
    localStorage.removeItem('CLORIT_WALLET_STATE');
  ...
  getWalletState: (): WalletState | null => {
    try {
      const state = localStorage.getItem('CLORIT_WALLET_STATE');
      if (state) return JSON.parse(state);

      // Migration/Fallback from old keys
      const oldState = localStorage.getItem('walletState');
      if (oldState) {
        const parsed = JSON.parse(oldState);
        localStorage.setItem('CLORIT_WALLET_STATE', oldState); // Migrate to new key
        localStorage.removeItem('walletState'); // Remove old key
        return parsed;
      }

      // Also check individual old keys for migration if no 'walletState' object was stored
      const oldConnected = localStorage.getItem('walletConnected');
      const oldAddress = localStorage.getItem('walletAddress');
      if (oldConnected === 'true' && oldAddress) {
        const migratedState: WalletState = {
          isConnected: true,
          address: oldAddress,
          chainId: '0x1', // Default or try to infer if possible
          success: true,
        };
        localStorage.setItem('CLORIT_WALLET_STATE', JSON.stringify(migratedState));
      ...
      console.error('Error getting wallet state:', e);
      return null;
    }
  },
  ...
  isConnected: (): boolean => {
    return localStorage.getItem('CLORIT_WALLET_CONNECTED') === 'true';
  },
